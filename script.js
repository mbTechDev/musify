...
// ===== MUSIC FILTER =====
let musicFilter = [
  { category: "Musify Custom Collection", icon: "./edit.gif" },
  {
    category: `Love Mashup ${new Date().getFullYear()}`,
    icon: "./in-love.gif",
  },
  { category: "Hip Hop Rap Songs", icon: "./hipHop.gif" },
  { category: "Lo-Fi Chill Beats", icon: "./relaxed.gif" },
  { category: "Bollywood Remix Hits", icon: "./crown.gif" },
  {
    category: `Heartbreak Mashup ${new Date().getFullYear()}`,
    icon: "./broken-heart.gif",
  },
];
  ...
  musicFilterLi.addEventListener("click", function () {
    if (item.category === "Musify Custom Collection") {
      ...
      initialSetup(
        ...
        "#fff",
        "#000",
        `<img class="w-[24px] mr-[7px]" src="./music.gif" /> All Musics`
      );
...
favoriteMusics.addEventListener("click", function () {
  ...
  if (isClicked === false) {
    ...
    initialSetup(
      ...
      "#262626",
      "#fff",
      `<img src="./favorite.gif" class="w-[24px] mr-[7px]"/> Favorite`
    );
  }
});
